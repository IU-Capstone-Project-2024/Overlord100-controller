...
//         double angular_right = (2 * linear_velocity + (robot_base_ / 2) * angular_velocity) / (2 * wheel_radius_);
//         double angular_left = (2 * linear_velocity - (robot_base_ / 2) * angular_velocity) / (2 * wheel_radius_);
...
//         RCLCPP_INFO(this->get_logger(), "Publishing wheels_msg.left: '%s'", std::to_string(wheels_msg.left).c_str());
//         RCLCPP_INFO(this->get_logger(), "Publishing wheels_msg.right: '%s'", std::to_string(wheels_msg.right).c_str());
//         RCLCPP_INFO(this->get_logger(), "Robot base '%d'", this->get_parameter("robot_base_", robot_base_));
...
private:
    ...
    // Parameters for the differential drive robot
    double robot_base_ = 0.575;
    double wheel_radius_ = 0.0655;
    double half_robot_base_ = 0.2875;
};
